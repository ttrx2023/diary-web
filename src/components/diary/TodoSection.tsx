...
function SortableTodoItem({ item, onToggle, onRemove, onUpdateText, onBlur }: SortableTodoItemProps) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "group bg-card md:bg-background rounded-lg border shadow-sm hover:shadow-md transition-all duration-200",
    ...
    >
      <div className="flex items-center gap-2 p-3">
        ...
        <Input
          ...
          onBlur={onBlur}
          className={cn(
            "flex-1 border-0 bg-transparent shadow-none px-1 h-7 text-sm focus-visible:ring-0",
      ...
      <CardContent className="p-0 flex flex-col flex-1 min-h-0">
        ...
        {!isLoading && todos.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 md:py-6 text-center space-y-2 flex-1">
            <div className="p-2 bg-purple-500/10 rounded-full">
              <CheckSquare className="h-5 w-5 text-purple-500/50" />
            </div>
        ...
        {!isLoading && todos.length > 0 && (
          <div className="relative flex-1 min-h-0">
            ...
            {/* Fade overlay for scroll indication - Desktop only */}
            {todos.length > 5 && (
